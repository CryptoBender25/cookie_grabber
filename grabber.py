...
path = str(input("Перетяните файл хромдрайвера "))
...
def mainth(data):
	...
	for _ in range(3):
		try:
			...
			driver = webdriver.Chrome(path, seleniumwire_options=options, options = co)
			...
			for _ in range(20):
				try:
					logger.info("Trying")
					driver.find_element(By.XPATH, '//input[inputmode()="tel"]').send_keys(two_fa_info)
			...
			for _ in range(20):
				try:
					logger.info("Trying")
					driver.find_element(By.XPATH, '//input[@autocomplete="tel"]').send_keys(two_fa_info)
					wait.until(EC.element_to_be_clickable((By.XPATH, '//span[text()="Next"]'))).click()
